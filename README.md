...
Скопируйте `config.example.yaml` в `config.yaml`. Сначала проверяется текущая
папка, затем папка рядом с `bpdf.exe`. Другой путь задаётся через
`--config file.yaml`. Парсер намеренно поддерживает только плоский набор
`ключ: значение`, чтобы не тянуть полноценную YAML-библиотеку.
