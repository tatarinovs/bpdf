...
- **Groq Vision OCR:** Требуется Groq API Key (указывается в `config.jsonc` или аргументе команды).
...
- `--ffmpeg <PATH>` — Путь к `ffmpeg.exe`.
- `--force` — Разрешить перезапись исходных файлов (необходимо, если результат конвертации совпадает с путем исходника).
...
3. **Конвертация (`convert`):** Если выравнивание путей приводит к перезаписи исходного файла (например, `bpdf convert photo.jpg`), требуется обязательный флаг `--force`. Без него операция завершится ошибкой.
