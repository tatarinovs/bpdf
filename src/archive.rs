...
pub fn load_cbz(path: &Path, options: &ImageOptions, page_size: Option<&str>) -> Result<Document> {
    let file = File::open(path).with_context(|| format!("failed to open CBZ archive {}", path.display()))?;
    let mut archive = ZipArchive::new(file).with_context(|| format!("failed to parse ZIP archive {}", path.display()))?;

    let mut image_names = Vec::new();
    for i in 0..archive.len() {
        let entry = archive.by_index(i).with_context(|| format!("failed to read ZIP entry index {i}"))?;
        let name = entry.name();
    ...
    for name in image_names {
        let mut entry = archive.by_name(&name).with_context(|| format!("failed to read archive entry {name}"))?;
        let mut bytes = Vec::new();
        entry.read_to_end(&mut bytes).with_context(|| format!("failed to extract image {name}"))?;
